...
- **A named parameter, not an `extra` entry.** Typing its own platform's arguments is the one job a layer-3 client exists to do; `extra` stays the escape hatch for an extension this client does not know about (a third-party server, or a newer version of ours). That split, and the reserved-key guard behind it, are normative in the workspace spec `docs/specs/pipelex-platform-api.md` → "Layered extension policy" (Rule 5). It was previously passed as `extra={"method_id": …}`, which worked but documented nothing and validated nothing.
