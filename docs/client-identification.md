...
- `python/<major.minor.micro> (<os>; <arch>)` reads `sys.version_info`, `platform.system().lower()` and `platform.machine()`. When the platform values are not valid tokens, the comment is dropped and the runtime token stays.
