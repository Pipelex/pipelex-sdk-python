...
- **`write_codegen_tree`, the verbatim codegen tree writer**: `pipelex_sdk.codegen_writer.write_codegen_tree(report, output_dir=…)` writes a valid `codegen()` response to disk byte for byte — every artifact at its `path`, the lock as `codegen.lock` — so a Python project regenerates a tree identical to a local `pipelex codegen types` run without installing `pipelex`. Like that command it validates every path before writing, including the paths it is about to prune and the response's lock, which must track exactly its artifacts, raises `CodegenError` rather than overwrite a file codegen does not own, rewrites only what changed, and prunes stamped artifacts the previous lock tracked; the lock format and path rules it reads are public in `pipelex_sdk.codegen_lock` and `pipelex_sdk.codegen_stamp`.
