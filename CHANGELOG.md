...
- **`SyncPipelexAPIClient`**: a synchronous facade over `PipelexAPIClient` (`from pipelex_sdk.sync_client import SyncPipelexAPIClient`), so a script, a batch job or a Django view calls every method without `await` or an `asyncio.run` wrapper of its own, with the same signatures, results and errors, and the paged iterators as plain iterators. It delegates to one private async client on its own event-loop thread, so one instance can be shared across threads, and a call made inside a running event loop raises the new `SyncClientInEventLoopError` instead of freezing that loop.
