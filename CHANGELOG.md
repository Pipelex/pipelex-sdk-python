...
- **A list's item narrows on the nameless layer, and a consumer following the recursion has to move with it.** `mthds` 0.10.0 split the input-form field union by whether a node names itself: a top-level field stays the named union (`TextField`, `DocumentField`, …), each now requiring `name: str`, while a `ListField.item` parses into the new nameless union (`TextItem`, `DocumentItem`, …), which refuses a `name`. Code that narrowed a list's item with `isinstance(node.item, DocumentField)` — the shape this SDK's own docs recommended — now takes the false branch, silently rather than loudly, since `DocumentField` still exists and still imports. Narrow to `DocumentItem` instead. Because each `*Field` derives from its `*Item`, the item layer is the only safe narrowing target in that position: a check against the named layer is the one that goes wrong. (Breaking)
- **The input form's own parse tightened, so a report that used to ride through can now be refused.** Through the same floor move, the standard's client rejects a top-level field whose `required` contradicts its `presence` (`required: true` with `presence: "optional"`, or `required: false` with `plain`/`force`), a field combining `gating: true` with an `optional` presence marker — an omissible slot can never block a run — and an explicit `null` on any input-form wire slot except `default_value`, where `null` means "no default" on purpose. These are parse-time refusals inside `validate`'s response, not client-side validation of anything a caller passes, and the hosted API emits conformant descriptors. (Breaking)
- Bumped the `mthds` dependency floor from `>=0.8.2` to `>=0.11.0`, spanning the release that publishes both artifacts as normative pages with client models (0.9.0), the one that splits the input-form union and tightens its coherence rules (0.10.0), and the one that rejects `gating` on an optional slot (0.11.0). The floor is raised to the latest published release deliberately, so the version this SDK tests against is the version it demands of the environments it lands in — `pipelex` moves its own floor the same way, and the resolver takes the higher of the two when both are installed.
