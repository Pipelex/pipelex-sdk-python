...
- **Async-native, with one sync facade that delegates.** `PipelexAPIClient` is httpx `AsyncClient` and `async def` throughout. `SyncPipelexAPIClient` (`pipelex_sdk/sync_client.py`) wraps it through one private event loop on a daemon thread and is never a second transport: no retry, error-mapping, paging or polling logic is restated there. A public coroutine added to `PipelexAPIClient` gets its sync twin, with the identical signature, in the same change; `tests/unit/test_sync_client_parity.py` fails otherwise.
