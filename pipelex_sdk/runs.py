...
These types are **owned by this SDK** (not imported from `mthds`): the run
lifecycle is a Pipelex-branded hosted surface, mirroring `pipelex-sdk-js/src/runs.ts`.
During the transition (HANDOFF Phase 2) the same shapes still exist in
`mthds-python`; that duplication is deliberate and is removed from `mthds-python`
in Phase 6, leaving these as the single home.
