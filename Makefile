...
make agent-test               - Run unit tests quietly (only prints on failure)
...
make check                    - Shorthand -> cleanderived format lint pyright mypy check-unused-imports pylint
make agent-check              - Shorthand -> fui format lint pyright mypy
...
install: env-verbose
	...
	uv sync --all-extras && \
	echo "Installed $(PROJECT_NAME) dependencies in ${VIRTUAL_ENV} with all extras.";
...
pylint: env
	...
	$(VENV_PYLINT) --rcfile pyproject.toml pipelex_sdk tests
